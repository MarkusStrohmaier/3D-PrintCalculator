...
# --- VERZIERTES PDF DESIGN ---
def create_styled_pdf(project):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_margins(20, 20, 20)
    
    # Farben definieren (Dunkelblau)
    accent_color = (26, 64, 102) # RGB
    
    # Header: MaxlDruck
    pdf.set_font("Arial", 'B', 38)
    pdf.set_text_color(*accent_color)
    pdf.cell(0, 20, "MaxlDruck", ln=True)
    
    # Untertitel
    pdf.set_font("Arial", '', 12)
    pdf.set_text_color(50, 50, 50)
    pdf.cell(0, 10, "MATERIALPREISLISTE / KALKULATION", ln=True)
    
    # Datum und Nr (Oben Rechts)
    ...
    pdf.cell(0, 10, f"PROJEKT: {project.name.upper()}", ln=True)
    pdf.ln(5)
    ...
    pdf.cell(90, 10, " BESCHREIBUNG", fill=True)
    pdf.cell(25, 10, "ANZAHL", fill=True, align='C')
    pdf.cell(25, 10, "PREIS", fill=True, align='R')
    pdf.cell(30, 10, "SUMME ", fill=True, align='R', ln=True)
    ...
    # Gesamtsumme Bereich (KORRIGIERT)
    pdf.ln(15)
    pdf.set_font("Arial", 'B', 14)
    pdf.set_text_color(*accent_color)
    
    # Wir nutzen cell mit definierter Breite statt den Text zu quetschen
    pdf.cell(130, 10, "GESAMTSUMME", align='R')
    
    pdf.set_font("Arial", 'B', 20) # Etwas kleiner als 24, damit es besser passt
    pdf.set_text_color(0, 0, 0)
    pdf.cell(40, 10, f"{total_sum:.2f} EUR", align='R', ln=True)
    
    # Abschlusslinie (Position dynamisch an den Text angepasst)
    current_y = pdf.get_y()
    pdf.set_draw_color(*accent_color)
    pdf.set_line_width(1.0)
    pdf.line(130, current_y, 190, current_y)
    
    return pdf.output(dest='S').encode('latin-1', 'replace')

# --- STREAMLIT UI ---
st.set_page_config(page_title="MaxlDruck Manager", layout="wide")
...
with st.sidebar:
    ...
    with st.expander("Material & Drucker"):
...
with col_form:
    ...
    with st.container(border=True):
        ...
        if i_type == "Druckteil":
            ...
            if not m_db or not p_db:
                st.warning("Bitte zuerst Materialien und Drucker in der Sidebar anlegen!")
            else:
                c1, c2 = st.columns(2)
                sel_m = c1.selectbox("Material", list(m_db.keys()))
                sel_p = c2.selectbox("Drucker", list(p_db.keys()))
                c3, c4 = st.columns(2)
                i_w = c3.number_input("Gewicht (g)", min_value=0.0, value=float(edit_val.get('Gewicht (g)', 0.0)))
                i_t = c4.number_input("Zeit (h)", min_value=0.0)
            ...
            c1, c2 = st.columns(2)
            i_pr = c1.number_input("Preis/Stk", min_value=0.0)
            i_q = c2.number_input("Anzahl", min_value=1, value=1)
            if st.button("Übernehmen" if is_item_edit else "Hinzufügen"):
...
with col_list:
    ...
    for idx, itm in enumerate(st.session_state.current_items):
        with st.container(border=True):
            ...
            cols[0].write(f"**{itm['Name']}** ({itm['Details']})")
            cols[0].caption(f"{itm['Kosten (€)']} EUR")
    ...
    if st.session_state.current_items:
        ...
        if st.button("💾 PROJEKT FINAL SPEICHERN", use_container_width=True, type="primary"):
            ...
            new_p = Project(name=st.session_state.p_name)
...
for proj in db.query(Project).order_by(Project.id.desc()).all():
    with st.expander(f"📦 {proj.name.upper()} - {sum(i.cost for i in proj.items):.2f} EUR"):
        ...
        with c1:
            if st.button("✏️ Projekt Laden", key=f"lp_{proj.id}"):
                ...
                st.rerun()
        with c2: st.download_button("📄 PDF Rechnung", data=create_styled_pdf(proj), file_name=f"MaxlDruck_{proj.name}.pdf", key=f"pdf_{proj.id}")
        with c3:
